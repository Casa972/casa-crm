  ...
  return (
    <Document>
      ...
      <Page size="A4" style={s.page}>
        ...
        <View>
          <View style={s.row}><Text style={s.lab}>Adresse</Text><Text style={s.val}>{[e.adresse, `${e.codePostal} ${e.commune}`].filter(Boolean).join(", ")}</Text></View>
          <View style={s.row}><Text style={s.lab}>Référence cadastrale</Text><Text style={s.val}>{[e.sectionCadastrale && `Section ${e.sectionCadastrale}`, e.parcelle && `Parcelle n° ${e.parcelle}`].filter(Boolean).join(" — ") || "—"}</Text></View>
          <View style={s.row}><Text style={s.lab}>Superficie du terrain</Text><Text style={s.val}>{fmtM2(e.superficieTerrain)}{e.zonagePlu ? ` — Zonage PLU : ${e.zonagePlu}` : ""}</Text></View>
          <View style={s.row}><Text style={s.lab}>Nature du bien</Text><Text style={s.val}>{e.natureBien || "—"}</Text></View>
          <View style={s.row}><Text style={s.lab}>Surface de plancher (SHON)</Text><Text style={s.val}>{fmtM2(e.surfaceShon)}</Text></View>
          <View style={s.row}><Text style={s.lab}>Emprise au sol (construction)</Text><Text style={s.val}>{fmtM2(e.empriseSol)}</Text></View>
        </View>
        <SH n={3} title="DESCRIPTION DU BIEN" />
      ...
      <Page size="A4" style={s.page}>
        ...
        <SH n={4} title="LOCALISATION ET ENVIRONNEMENT" />
        ...
        <SH n={5} title="ESTIMATION DE LA VALEUR LOCATIVE" />
        {e.analyseMarche.split("\n").filter(Boolean).map((p, i) => (<Text key={i} style={s.body}>{p}</Text>))}
        <Footer />
      </Page>
      <Page size="A4" style={s.page}>
        ...
        <Text style={s.body}>{e.disclaimer}</Text>
